...
DEFAULT_OPENAI_ROAD_IMAGE_EDIT_PROMPT = (
    "Edit the input image minimally. Keep the road scene as close as possible to the original frame.\n\n"
    "Generate a clean road background. Remove all vehicles, road users, distant small cars, vehicle shadows, "
    "reflections, ghosting, and residual artifacts from the image.\n\n"
    "Preserve the exact camera perspective, road geometry, lane width, lane markings, curbs, sidewalks, "
    "buildings, lighting, texture, and natural environmental style. Only fill the regions that were occluded "
    "by vehicles or other road users.\n\n"
    "Avoid over-cleaning, avoid replacing the environment, avoid changing road geometry, and avoid changing "
    "the scene style."
)
...
def generate_target_frame_background_assets(
    *,
    ...
    road_clean_mode = not tabletop_task_mode
    openai_image_cleaner_enabled = tabletop_task_mode and cleaner_name in {
        "openai_image_edit",
        "gpt_image_edit",
        "gpt-image-edit",
    }
    limit_frames = [] if (openai_image_cleaner_enabled or road_clean_mode) else (
    ...
    if tabletop_task_mode:
        ...
        if openai_image_cleaner_enabled:
            ...
            edited_rgb, raw_path, cleaner_result = _run_background_image_cleaner(
                ...
                raw_output_path=raw_clean_path,
                cleaner_config=cleaner_config,
                reference_frame_id=reference_frame_id,
                background_image_cleaner=background_image_cleaner,
            )
    ...
    if road_clean_mode:
        ...
        confidence_path = output_dir / "confidence_map.png"
        source_count_path = output_dir / "source_count_map.png"
        cleaner_config = _load_openai_image_edit_config(background_cleaner_config)
        cleaner_config["prompt"] = DEFAULT_OPENAI_ROAD_IMAGE_EDIT_PROMPT
        cleaner_config["mask_mode"] = "prompt_only"
        cleaner_config["use_full_image_output"] = True
        reference_frame_id = int(background_cleaner_reference_frame_id or target_frame_id)
        reference_entry = _select_frame(frame_entries, reference_frame_id)
        reference_det = _load_json(reference_entry["detections"])
        reference_rgb = _decode_image_b64(reference_det["image_b64"])
        ...
        manifest = {
            "schema": (
                "guanwu.target_frame_background_assets.road_clean_depth.v2"
                if depth_asset
                else "guanwu.target_frame_background_assets.road_clean.v1"
            ),
            "target_frame_id": int(target_frame_id),
            "image_size": [int(width), int(height)],
            "assets": {
            ...
            "quality": {
                ...
                "clean_rgb_reference_frame_id": int(reference_frame_id),
                "clean_rgb_model": str((cleaner_result or {}).get("model") or cleaner_config.get("model") or "gpt-image-2"),
                "clean_rgb_prompt": str((cleaner_result or {}).get("prompt") or cleaner_config.get("prompt") or DEFAULT_OPENAI_ROAD_IMAGE_EDIT_PROMPT),
                "clean_rgb_mask_fraction": 0.0,
                "clean_rgb_mask_mode": "prompt_only",
                "clean_rgb_mask_frame_mode": "reference_frame",
                "clean_rgb_full_image_output": True,
            },
            "road_plane": None,
        }
        if depth_asset:
            manifest["assets"].update({key: value for key, value in depth_asset.get("assets", {}).items() if value})
            manifest["quality"].update(depth_asset.get("quality", {}))
        manifest_path = output_dir / "background_manifest.json"
        manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
        return {"manifest_path": str(manifest_path), "mesh_dir": str(mesh_dir)}

    raise RuntimeError("Unreachable background mode: non-tabletop backgrounds use road_clean_background.")


def _run_background_image_cleaner(
    *,
    reference_rgb: np.ndarray,
    cleaner_mask: np.ndarray,
    ...
    background_image_cleaner: Callable[..., dict[str, Any] | None] | None,
) -> tuple[np.ndarray, Path, dict[str, Any] | None]:
    height, width = output_shape
    Image.fromarray(reference_rgb).save(reference_path)
    if prompt_only_mode:
        Image.fromarray(np.zeros((height, width, 4), dtype=np.uint8), mode="RGBA").save(mask_path)
    else:
        _write_openai_edit_mask_png(mask_path, cleaner_mask)
    ...
    raw_path = Path((cleaner_result or {}).get("raw_output_path") or (cleaner_result or {}).get("clean_rgb_path") or raw_output_path)
    ...
    return edited_rgb, raw_path, cleaner_result
